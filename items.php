...
//SQL
	...
	$itmqry = 'SELECT items.id, items.org_id, items.desc, items.value, items.itemnum, items.received, items.location, items.dt, members.usr FROM items INNER JOIN members ON items.usr_id=members.id WHERE org_id = '.$orgid.' ORDER BY items.dt DESC';
