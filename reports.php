...
//SQL
	...
	//$clsldrqry = 'SELECT class.id, class.name, class.numkids, SUM(items.value) AS total, (class.numkids*50) AS goal FROM org JOIN members ON org.usr_id = members.id JOIN items ON items.org_id = org.id JOIN class ON members.class_id = class.id GROUP BY class.name ORDER BY total DESC';
				...
				<div class="container">
					<div class="row">
						<div class="span12">
							...
							?>
							<!--<h4>Solicitation Progress: <?=$persol;?>%</h4>
							...
							<div class="progress">
								<div class="progress bar" data-appear-progress-animation="<?php echo $persol; ?>%" style="width: <?php echo $persol; ?>%;">
						...
						<div class="span12">
							...
							<div class="tabs">
								<ul class="nav nav-tabs">
									<li class="<?php echo $classtab; ?>"><a href="#classldr" data-toggle="tab"><i class="icon-group"></i> Classroom Leaderboard (<?=mysql_num_rows($clsldrres);?>)</a></li>
									<li class="<?php echo $parenttab; ?>"><a href="#parentldr" data-toggle="tab"><i class="icon-user"></i> Parent Leaderboard (<?=mysql_num_rows($prtldrres);?>)</a></li>
									<li class="<?php echo $rcvtab; ?>"><a href="#itemsrcv" data-toggle="tab"><i class="icon-tags"></i> Items Received (<?=mysql_num_rows($rcvres);?>)</a></li>
									<li class="<?php echo $yestab; ?>"><a href="#donateyes" data-toggle="tab"><i class="icon-thumbs-up"></i> Will Donate (<?=mysql_num_rows($yesres);?>)</a></li>
									<li class="<?php echo $notab; ?>"><a href="#donateno" data-toggle="tab"><i class="icon-thumbs-down"></i> Will Not Donate (<?=mysql_num_rows($nores);?>)</a></li>
								</ul>
											...
											<tbody>
												<?php
													...
													//parent participation points earned
														...
														echo '<tr>';
														echo '	<td>'.mb_substr($row['fname'], 0, 1, 'utf-8').'.&nbsp'.$row['lname'].'</td>';
														echo '	<td>$'.$row['total'].'</td>';
